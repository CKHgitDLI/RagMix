...
class Dealer:
    ...
    def search(self, req, idxnms, emb_mdl=None, highlight=False):
        ...
        print("【Q】: {}".format(json.dumps(s)))
        res = self.es.search(deepcopy(s), idxnms=idxnms, timeout="600s", src=src)
        print("TOTAL: {}".format(self.es.getTotal(res)))
    ...
    def retrieval(self, question, embd_mdl, knowledgebase_name, page, page_size, similarity_threshold=0.2,
        ...
        if isinstance(knowledgebase_name, str):
            knowledgebase_name = knowledgebase_name.split(",")
        print(req)
        sres = self.search(req, knowledgebase_name, embd_mdl, highlight)
