...
async def response(data: Dict):
    ...
    ref = re.run(query=ask, embd_mdl=ollama_embedding, rerank_mdl=None,
                 ...
                 keywords_similarity_weight=0.5, top_n=20, top_k=1024, empty_response="", knowledgebase_name="ckh")
    ref_s = ""
    # 召回数据处理
    ...
    print(unique_list)
    reference=""
    for i in range(len(unique_list[:4])):
    ...
    return EventSourceResponse(ge.stream_output(history=[{"role": "user", "content": ask}], chat_mdl=ollama_chat,
                                                retrieval_res=None,
                                                embd_mdl=None,max_tokens=512000,
prompt="""
# 以下内容是基于用户发送的消息的搜索结果:
{results}
在回答时，请注意以下几点:
